...
const ManageShop = props => {
    ...
    const stripeLoaded = useMemo(() => {
        return 'charges_enabled' in stripe && 'details_submitted' in stripe;
    }, [stripe])
    ...
    const disabled = {filter: 'brightness(.7)', pointerEvents: 'none'};

    const style = stripe.charges_enabled && stripe.details_submitted ? {} : disabled;
        ...
        <div className='my-shop manage'>
            <header className='my-shop manage header'>
                ...
                <ul className='my-shop manage header actions'>
                    ...
                    <Link onClick={e => e.preventDefault()} to={stripe.charges_enabled && stripe.details_submitted ? '/my-shop/sales' : '#'} id='sales' title='Sales: Create sales and discounts for your products and items, coming soon!' style={disabled} className={`my-shop manage header actions item${page === 'sales' ? ' active' : ''}`}>                        
            ...
            <section className='my-shop manage main'>
                <div className='notifications'>
                    ...
                    timeout={500}
                    classNames={'fade'}
                    in={stripeLoaded && !stripe.charges_enabled}
                    mountOnEnter={true}
                    unmountOnExit={true}
                    >
                    ...
                    timeout={500}
                    classNames={'fade'}
                    in={stripeLoaded && !stripe.details_submitted}
                    mountOnEnter={true}
                    unmountOnExit={true}
                    >
                        <div className='item' onClick={completeStripeRegistration}>
                    ...
                    timeout={500}
                    classNames={'fade'}
                    in={stripeLoaded && (!stripe.details_submitted || !stripe.charges_enabled)}
                    mountOnEnter={true}
                    unmountOnExit={true}
                    >
                        <div className='item' onClick={completeStripeRegistration}>
