...
const UserActions = props => {
    ...
    return (
        <ul className='user'>
            {
                !loggedIn ? (
                    <li className='user item sign'>
                        <Link to='/login' title='Sign In'>Login</Link> or <Link title='Register' to='/register'>Register</Link>
                    </li>
                ) : (
                    <li className='user item auth' onMouseEnter={() => setDropdown(true)} onMouseLeave={() => setDropdown(false)}>
                        ...
                        <CSSTransition timeout={500} classNames={type !== 'menu' ? 'fade' : 'expand'} in={dropdown} mountOnEnter={true} unmountOnExit={true}>
                            <ul className='user item auth dropdown'>
                                <li>
                                    <Link className='disabled' onClick={e => e.preventDefault()} to='/account' title='Account: View and edit your account, coming soon!'>
                                ...
                                <li onClick={() => auth.logout(dispatch, login)}>
                                    <Link to='#'>
